...
if __name__ == '__main__':
    ...
    if file1 is None or file2 is None:
        ...
        detector = MTCNN()
        image1 = detector.detect_faces(np.array(image1))
        image2 = detector.detect_faces(np.array(image2))
        if image1 == [] or image2 == []:
            st.write('Please upload two images with faces.')
        else:
            st.image(image1, caption='Uploaded Image.', use_column_width=True)
            st.image(image2, caption='Uploaded Image.', use_column_width=True)
            st.write("")
            st.write("Classifying...")
            image1 = PIL.Image.fromarray(image1[0]['box'])
            image2 = PIL.Image.fromarray(image2[0]['box'])
